# TODO: Translation updated at 2022-08-27 18:12

translate english strings:

    # game/script.rpy:12
    old "Рейму"
    new "Reimu"

    # game/script.rpy:13
    old "Мариса"
    new "Marisa"

    # game/script.rpy:14
    old "Бякурен"
    new "Byakuren"

    # game/script.rpy:15
    old "Рин"
    new "Rin"

    # game/script.rpy:16
    old "Ринноске"
    new "Rinnosuke"

    # game/script.rpy:17
    old "Сумиреко"
    new "Sumireko"

    # game/script.rpy:18
    old "Сакуя"
    new "Sakuya"

    # game/script.rpy:19
    old "Юкари"
    new "Yukari"

    # game/script.rpy:20
    old "Моко"
    new "Mokou"

    # game/script.rpy:21
    old "Канако"
    new "Kanako"

    # game/script.rpy:22
    old "Ёму"
    new "Youmu"

    # game/script.rpy:23
    old "Санаэ"
    new "Sanae"
# TODO: Translation updated at 2022-10-31 16:42

translate english strings:

    # game/script.rpy:43
    old "{size=50}Это фанатская игра по вселенной Touhou Project.\nВсе права принадлежат Team Shanghai Alice.{/size}"
    new "{size=50}This is a fan game based on the Touhou Project.\nAll rights reserved by Team Shanghai Alice.{/size}"
# TODO: Translation updated at 2023-04-14 21:01

translate english strings:

    # game/script.rpy:24
    old "Кокоро"
    new "Kokoro"

    # game/script.rpy:25
    old "Мамидзо"
    new "Mamizou"

    # game/script.rpy:26
    old "Касэн"
    new "Kasen"

    # game/script.rpy:27
    old "Клаунпис"
    new "Clownpiece"

    # game/script.rpy:28
    old "Эйрин"
    new "Eirin"

    # game/script.rpy:29
    old "Рейсен"
    new "Reisen"

    # game/script.rpy:30
    old "Назрин"
    new "Nazrin"

    # game/script.rpy:33
    old "Баке-дануки"
    new "Bake-danuki"

    # game/script.rpy:34
    old "Кролик"
    new "Rabbit"

    # game/script.rpy:35
    old "Тэнгу"
    new "Tengu"

    # game/script.rpy:36
    old "Итимэ-кодзо"
    new "Itime-kozo"

    # game/script.rpy:37
    old "Каппа"
    new "Kappa"

# TODO: Translation updated at 2023-07-05 04:57

translate english strings:

    # game/script.rpy:57
    old "Я хочу играть на Русском"
    new "Я хочу играть на Русском"

    # game/script.rpy:57
    old "I want to play in English"
    new "I want to play in English"



# TODO: Translation updated at 2023-07-07 12:58

translate english strings:

    # game/script.rpy:82
    old "{size=50}Внимание! Игра содержит сцены насилия и смерти и не рекомендуется лицам младше 16 лет!{/size}"
    new "{size=50}Warning! The game contains scenes of violence and death and is not recommended for persons under 16 years old!{/size}"

# TODO: Translation updated at 2023-07-26 18:13

translate english strings:

    # game/script.rpy:57
    old "Eu quero jogar em Português"
    new "Eu quero jogar em Português"

# TODO: Translation updated at 2023-08-31 11:34

translate english strings:

    # game/script.rpy:54
    old "{font=fonts/SourceHanSansCN-Regular.otf}以简体中文开始{/font}"
    new "{font=fonts/SourceHanSansCN-Regular.otf}以简体中文开始{/font}"

    # game/script.rpy:54
    old "{font=fonts/SourceHanSansTC-Regular.otf}以繁體中文開始{/font}"
    new "{font=fonts/SourceHanSansTC-Regular.otf}以繁體中文開始{/font}"

